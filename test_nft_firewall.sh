...
run_case() {
    local name="$1" ruleset="$2" chain="$3" expect_rc="$4" expect_insert="$5"
    NFT_RULESET="$ruleset"
    NFT_CHAIN="$chain"
    NFT_INSERTS=0
    PERSIST_CALLED=0
...
run_case "fail2ban accept 链无需误报失败" "$F2B_RULESET" "$F2B_RULESET" 0 0
run_case "drop input 链会 insert 放行规则" "$DROP_RULESET" "$DROP_RULESET" 0 1
